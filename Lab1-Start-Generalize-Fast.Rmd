...
Explanation: Under the control, average waste piles is 77.91 and under treatment it has been reduced by 24.77
...
```{r}
# From performance package, quick way to get qq plot
check_model(m1_ols,  check = "qq" )
```

Explanation: Probably not normally distributed
...
exp(-.38)-1
# % change in the outcome that is predicted = 31.6% reduction 

```

**Check overdispersion assumption**

Variance in the poisson model we call dispersion. 
...
- Exponentiation of the beta coefficient () FILL IN THE MISSING TEXT  
...
Study did use randomization, group by contiguous neighborhoods (so they could compete). Fairly small sample. Usually will see slight bias unless sample size is very large. Always need to check for co variant balance. May be select bias or ommited variable bias. 
------------------------------------------------------------------------
...
Control estimate is small, could be significant. It adjusts our treatment effect. 
