...
exp(-.38) -1
```
This model predicts a 32% reduction in waste piles 
...
```
It is over dispersed. The dispersion is greater than the mean. 
...
exp(-0.42) - 1
```
This model predicts a 34% reduction in waste piles. 
...
First two models are underestimating the treatment effect. 
...
m4_control <- lm(
    log(waste_piles) ~ 
        treat +
        log(pre_count_avg), ## Add a control variable 
    data = post_treat_subset)
...
Treatment effect changed significantly with the inclusion of this control. 
...
Rain can be thought of a different kind of treatment that effects waste pile burning
...
m5_control2 <- lm(
    log(waste_piles) ~ 
        treat +
        log(pre_count_avg) +
        rain_0_48hrs + rain_49_168hrs, # add these two control predictors 
    data = post_treat_subset)

summ(m5_control2,
     model.fit = FALSE,
     robust = "HC2")  # Heteroskedasticity robust standard error adj. (makes standard error more conservative)
```

We get a very similar result. Robust and stable after adding the two control variables. 
...
Neighborhoods are paired together because they are close, and this means they are likely more similar to each other than to other neighborhoods. This means there is likely a higher correlation between paired neighborhoods. 

Simpson's paradox. We need to account for this correlation in clustered data. 
